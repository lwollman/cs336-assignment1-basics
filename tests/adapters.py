...
def run_rmsnorm(
    ...
    """
    rmsnorm_instance = homework.RMSLayerNormalization(
        d_model=d_model,
        eps=eps
    )
...
def run_get_batch(
    ...
    """
    return homework.get_batch(dataset, batch_size, context_length, device)
...
def run_cross_entropy(
    ...
    return homework.cross_entropy(inputs, targets)
    # raise NotImplementedError
...
def run_load_checkpoint(
    ...
    Args:
        src: the checkpoint_path (str | os.PathLike | BinaryIO | IO[bytes]): Path or file-like object to serialized checkpoint.
        model (torch.nn.Module): Restore the state of this model.
        optimizer (torch.optim.Optimizer): Restore the state of this optimizer.
    Returns:
        int: the previously-serialized number of iterations.
    """
    return homework.load_checkpoint(
        src=src,
        model=model,
        optimizer=optimizer,
    )
